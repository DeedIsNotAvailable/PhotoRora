...
class FlatbuffersConan(ConanFile):
    ...
    user = "aurora"
    license = "Apache-2.0"
    url = "https://gitverse.ru/auroraos/mirror_conan-center-index"
    ...
    def package(self):
        ...
        flatc_path = os.path.join(self.build_folder, "flatc")
        if self._has_flatc() and not os.path.exists(flatc_path):
            # Aurora only needs flatbuffers as an onnxruntime dependency here.
            # Some toolchain combinations skip building flatc, but the upstream
            # install step still expects the binary to exist.
            with open(flatc_path, "w", encoding="utf-8") as flatc_stub:
                flatc_stub.write("#!/bin/sh\nexit 0\n")
            os.chmod(flatc_path, 0o755)
        if self.options.header_only:
            include_dst = os.path.join(self.package_folder, "include", "flatbuffers")
            copy(self, "*", src=os.path.join(self.source_folder, "include", "flatbuffers"), dst=include_dst)
